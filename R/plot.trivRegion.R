...
fhat = x$kernel_fit
standarized_region = computeContour3d(fhat$estimate, maxvol = max(fhat$estimate),
                                        level = x$k_limit ,
  ...
  if(cond==FALSE){
    ...
    if(planes==TRUE){planes3d(10,0,0,col="azure");planes3d(0,10,0,col="azure");planes3d(0,0,10,col="azure")
    }
  }

  if(cond==TRUE){
    fhat = x$kernel_fit
    standarized_x = computeContour3d(fhat$estimate, maxvol = max(fhat$estimate),
                                     level = x$k_limit ,
                                     x = fhat$eval.points[[1]],
                                     y = fhat$eval.points[[2]],
                                     z = fhat$eval.points[[3]],
                                     mask=NULL)
    means = cbind(predict(x$fit$mean1,newdata=newdata),
                  predict(x$fit$mean2,newdata=newdata),
                  predict(x$fit$mean3,newdata=newdata))

    sds = cbind(sqrt(exp(predict(x$fit$var1$fit,newdata=newdata))),
                sqrt(exp(predict(x$fit$var2$fit,newdata=newdata))),
                sqrt(exp(predict(x$fit$var3$fit,newdata=newdata))))

    rhos = cbind(tanh(predict(x$fit$rho1$fit,newdata=newdata)),
    ...
    for(i in 1:nrow(newdata)){
      ...
      if(is.null(reg.col)){reg.col = colours_seq; reg.col_aux = colours_seq[i]}
      if(length(reg.col)>=1){reg.col_aux=reg.col[i]}
      ...
      if(add==TRUE){drawScene.rgl(makeTriangles(pol_pred,color=reg.col_aux,alpha=0.2),add = TRUE)}
    }
    ...
    legend3d("bottomright",legend=nomes,
