...
plot.bivRegion <- function(x, tau = 0.95,newdata=NULL, reg.col=NULL, reg.lwd=1,reg.lty=NULL, axes=TRUE,
  ...
  if(sum(tau%in%x$tau)!=length(tau)){return("WARNING: this coverage probability should be previously estimated with bivregion")}

  if(length(reg.col)==1){reg.col = rep(reg.col,length(tau))}
  if(length(reg.lwd)==1){reg.lwd = rep(reg.lwd,length(tau))}
  if(length(reg.lty)==1){reg.lty = rep(reg.lty,length(tau))}


  colours_seq = c(brewer.pal(n = 8, name = "Dark2"),
  ...
  if(is.null(reg.lty)){reg.lty = 1:length(tau)}
  if(is.null(reg.col)){reg.col = colours_seq}
    ...
    if(is.null(newdata)){
      ...
      plot(x$Y,...)
      for(i in 1:length(bb)){lines(x$region[[bb[i]]], col = reg.col, lwd = reg.lwd)}
    }else{
      plot(newdata[,names(x$Y)],...)
      bb = which((x$tau) %in% tau)
      for(i in 1:length(bb)){lines(x$region[[bb[i]]], col = reg.col[i], lwd = reg.lwd[i],lty=reg.lty[i])}
    }}
  ...
  if(!is.null(x$fit)&cond==FALSE){
    if(is.null(newdata)){
      ...
      if(legend==TRUE) legend("bottomright",col=reg.col[1:length(tau)],legend = tau,lwd=2.5,bty="n",lty = reg.lty[1:length(tau)])
        ...
        YC = as.data.frame(YC)
        names(YC) = c(all.vars(x$fit$mu1$formula)[1],
  ...
  if(is.null(newdata)){stop("newdata = NULL. This function was developed to represent the bivariate reference region at an specific covariate values. Please supply those values in the newdata argument.")}

    reg_pred = predict(x,cond=TRUE,newdata = newdata,tau = tau)

    if(add == TRUE){
      colours_seq2 = c(brewer.pal(n = 8, name = "Accent"),
                       brewer.pal(n = 8, name = "Set1"),
                       brewer.pal(n = 12, name = "Paired"))
    ...
    if(add == FALSE){
      ...
      plot(y1,y2,...)
      for(i in 1:length(reg_pred)){for(j in 1:length(reg_pred[[i]])) lines(reg_pred[[i]][[j]],col=colours_seq[j],lwd = reg.lwd,lty = reg.lty[i])}
      if(legend == TRUE){
