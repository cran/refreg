...
predict.bivRegion = function(object,tau=0.95,newdata=NULL,cond=FALSE,...){
  ...
  bb = which((object$tau) %in% tau);Out_points = list();Coverage = as.numeric()
  ...
  if(!is.null(object$fit)&!is.null(newdata)&cond==FALSE){
    ...
    newdata = na.omit(newdata[,unique(c(all.vars(object$fit$formula[[1]])[1],all.vars(object$fit$formula[[2]])[1],
    ...
    for(i in 1:length(bb)){
      ...
      names(Out_points[[i]]) = c("Both high",paste0(names(newdata_out)[1]," low and ",names(newdata_out)[2]," high"),
  ...
  if(!is.null(object$fit)&!is.null(newdata)&cond==TRUE){
    ...
    vars = unique(c(names(object$fit$mu1$model)[-1],names(object$fit$mu2$model)[-1],
    ...
    if(is.null(newdata)){
      R = predict(object$fit$rho); R[R>1.8]=1.8; R[R<(-1.8)]=-1.8;R = tanh(R)
    }else{
      R = predict(object$fit$rho,newdata=newdata); R[R>1.8]=1.8; R[R<(-1.8)]=-1.8;R = tanh(R)
    }
    ...
    for(j in 1:length(bb)){
      ...
      for(i in 1:dim(medias)[1]){
        ...
        pred_regions[[i]] = cbind(medias[i,1]+(t(apply(object$region[[bb[j]]],1,function(x) P%*%x)))[,1]*sds[i,1],
