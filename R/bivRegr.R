...
bivRegr <- function(f = f, data = data, ace.eps = 0.01, ace.itmax = 25) {
  data <- na.omit(data[, unique(c(
    all.vars(f[[1]])[1], all.vars(f[[2]])[1],
  ...
  # 2) Fit the models
  modelo_v1 <- ACE(
    y = "res1", predictor = f[[3]], restriction = "positive",
    eps = ace.eps, itmax = ace.itmax, data = data
  )
  modelo_v2 <- ACE(
    y = "res2", predictor = f[[4]], restriction = "positive",
    eps = ace.eps, itmax = ace.itmax, data = data
  )
  ...
  ret <- list(
    formula = f, bivres = YC, mu1 = modelo_m1, mu2 = modelo_m2, var1 = modelo_v1, var2 = modelo_v2,
