...
plot.summary_boot = function(x,eq=1,select=NULL,...){
  ...
  myplot <- function(data){
    if(class(data[,1]) =="factor"){
      g1 = ggplot(data, aes(x = data[,1], y = data[,2])) + geom_point(pch = 13,cex=5,col="#E41A1C") + theme_classic()
      ...
      if(eq==3){g1 <-g1+ ggtitle(paste0(names(x$fit$mu1$model)[1]," variance"))}
      if(eq==4){g1 <-g1 + ggtitle(paste0(names(x$fit$mu2$model)[1]," variance"))}
      if(eq==5){g1 <-g1 + ggtitle(paste0(names(x$fit$mu1$model)[1],"-",names(x$fit$mu2$model)[1]," correlation"))}
      g1
